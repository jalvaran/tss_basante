...
function anular_entrega_mipres(datos_mipres){
    console.log(datos_mipres);
    var idDivMensajes='sp_msg_mipres';
    document.getElementById(idDivMensajes).innerHTML='<div id="GifProcess">Anulando...<img   src="../../images/loader.gif" alt="Cargando" height="50" width="50"></div>';
    document.getElementById(idDivMensajes).innerHTML=document.getElementById(idDivMensajes).innerHTML+" "+datos_mipres["entrega_id"];
...
function obtenga_token_consulta_mipres(datos_mipres,funcion){
    var idDivMensajes='sp_msg_mipres';
    alertify.success("Obteniendo token");
    var form_data = new FormData();
        ...
        success: function(data){
            ...
            if(respuestas[0]=="OK"){                
                ...
                if(funcion==9){
                    consulte_reporte_entrega_mipres_x_rango(datos_mipres);
                }
                
                if(funcion==10){
                    anular_reporte_entrega_mipres(datos_mipres);
                }
                if(funcion==11){
                    reportar_entrega_mi_pres_x_id(datos_mipres);
                }
            }else if(respuestas[0]=="E1"){  
                alertify.error(respuestas[1],0);
                MarqueErrorElemento(respuestas[2]);
...
function consulta_entrega_mipres_x_prescripcion(datos_mipres){
    ...
    var form_data = new FormData();
        ...
        success: function(data){
            ...
            if(respuestas[0]=="OK"){                
                alertify.success(respuestas[1]);
                consulta_reporte_entrega_mipres_x_prescripcion(datos_mipres);
...
function consulta_reporte_entrega_mipres_x_prescripcion(datos_mipres){
    
    var idDivMensajes='sp_msg_mipres';
    document.getElementById(idDivMensajes).innerHTML='<div id="GifProcess">Consultando Reporte de Entrega X Prescripcion...<img   src="../../images/loader.gif" alt="Cargando" height="50" width="50"></div>';
    document.getElementById(idDivMensajes).innerHTML=document.getElementById(idDivMensajes).innerHTML+" "+datos_mipres["ID"];
...
function ListarReporteEntregaMipres(Page=1,datos_mipres=''){
    MuestraOcultaXID('div_filtro_mipres');
    var idDiv="DivGeneralDraw";
    ...
    var form_data = new FormData();
       ...
       $.ajax({// se arma un objecto por medio de ajax  
        ...
        success: function(data){            
            document.getElementById(idDiv).innerHTML=data; //La respuesta del servidor la dibujo en el div DivTablasBaseDatos                      
            dibuja_filtros_mipres(4,cmb_estado_mipres);
        },
        error: function (xhr, ajaxOptions, thrownError) {// si hay error se ejecuta la funcion
...
function iniciar_consulta_reporte_entrega_mipres(){
    delete datos_mipres;
    var idDivMensajes='sp_msg_mipres';
    ...
    var form_data = new FormData();
        ...
        success: function(data){
            ...
            if(respuestas[0]=="OK"){                
                ...
                datos_mipres["fecha_consulta"]=respuestas[4];
                datos_mipres["total_dias"]=respuestas[5];
                obtenga_token_consulta_mipres(datos_mipres,9);
            }else if(respuestas[0]=="E1"){  
                alertify.error(respuestas[1],0);
                MarqueErrorElemento(respuestas[2]);
...
function consulte_reporte_entrega_mipres_x_rango(datos_mipres){
    
    var idDivMensajes='sp_msg_mipres';
    document.getElementById(idDivMensajes).innerHTML='<div id="GifProcess">Consultando Reporte de Entrega Mipres...<img   src="../../images/loader.gif" alt="Cargando" height="50" width="50"></div>';
    document.getElementById(idDivMensajes).innerHTML=document.getElementById(idDivMensajes).innerHTML+" "+datos_mipres["fecha_consulta"];
...
function confirma_anular_reporte_entrega_mipres(reporte_entrega_id){
    alertify.confirm('Seguro que desea anular este reporte de entrega?',
        function (e) {
            if (e) {
                
                iniciar_anular_reporte_entrega_mipres_x_id(reporte_entrega_id);
            }else{
                alertify.error("Se canceló el proceso");
...
function iniciar_anular_reporte_entrega_mipres_x_id(reporte_entrega_id){
    delete datos_mipres;
    var datos_mipres=[];
    datos_mipres["reporte_entrega_id"]=reporte_entrega_id;
    obtenga_token_consulta_mipres(datos_mipres,10);
}    
    
function anular_reporte_entrega_mipres(datos_mipres){
    console.log(datos_mipres);
    var idDivMensajes='sp_msg_mipres';
    document.getElementById(idDivMensajes).innerHTML='<div id="GifProcess">Anulando Reporte de entrega...<img   src="../../images/loader.gif" alt="Cargando" height="50" width="50"></div>';
    document.getElementById(idDivMensajes).innerHTML=document.getElementById(idDivMensajes).innerHTML+" "+datos_mipres["entrega_id"];
    
    var form_data = new FormData();
        form_data.append('Accion', '18'); 
        form_data.append('token_consultas', datos_mipres["token_consultas"]);
        form_data.append('reporte_entrega_id', datos_mipres["reporte_entrega_id"]);
...
function frm_reporte_entrega_mipres(mipres_id){
    AbreModal('ModalAcciones');
    OcultaXID('BntModalAcciones');
    OcultaXID('btnCerrarModal');
    var idDiv="DivFrmModalAcciones";
...
function confirma_reporte_entrega_mipres(mipres_id){
    alertify.confirm('Seguro que desea reportar esta entrega?',
        function (e) {
            if (e) {
                
                iniciar_reporte_entrega_mipres_x_id(mipres_id);
            }else{
                alertify.error("Se canceló el proceso");
...
function iniciar_reporte_entrega_mipres_x_id(mipres_id){
    delete datos_mipres;
    var datos_mipres=[];
    datos_mipres["mipres_id"]=mipres_id;
    obtenga_token_consulta_mipres(datos_mipres,11);
}    
    
function reportar_entrega_mi_pres_x_id(datos_mipres){
    
    var idDivMensajes='sp_msg_mipres';
    document.getElementById(idDivMensajes).innerHTML='<div id="GifProcess">Reportando Entrega...<img   src="../../images/loader.gif" alt="Cargando" height="50" width="50"></div>';
    document.getElementById(idDivMensajes).innerHTML=document.getElementById(idDivMensajes).innerHTML+" "+datos_mipres["ID"];
    var mipres_valor_facturado=document.getElementById('mipres_valor_facturado').value;
        
    var form_data = new FormData();
        form_data.append('Accion', '19'); 
        form_data.append('token_consultas', datos_mipres["token_consultas"]);
        form_data.append('mipres_id', datos_mipres["mipres_id"]);
        form_data.append('mipres_valor_facturado', mipres_valor_facturado);
